	...
	'yaml',
	{
		provideCompletionItems(
			...
			if (isComposeFile(document)) {
				...
				if (
					new RegExp(
						/traefik\.http\.routers\.[\w_-]+\.tls.domains\[n\]\.$/
					).test(linePrefix)
				) {
				...
				if (
					new RegExp(/traefik\.http\.routers\.[\w_-]+\.rule=.+/).test(
						linePrefix
					)
				) {
...
const httpServices = vscode.languages.registerCompletionItemProvider(
	'yaml',
	{
		provideCompletionItems(
			...
			if (isComposeFile(document)) {
				if (
					new RegExp(/traefik\.http\.services\.[\w_-]+\.$/).test(linePrefix)
				) {
				...
				if (servicesRegex('loadbalancer.responseforwarding', linePrefix)) {
					return completeOptions(
						Object.keys(
							dockerKeys.http.services.loadbalancer.responseforwarding
						)
					);
				}
			}
		...
		provideCompletionItems(
			...
			if (isComposeFile(document)) {
				...
				if (middlewaresRegex('chain', linePrefix)) {
					return completeOptions(
						Object.keys(dockerKeys.http.middlewares.chain)
					);
				}
				...
				if (middlewaresRegex('errors', linePrefix)) {
					return completeOptions(
						Object.keys(dockerKeys.http.middlewares.errors)
					);
				}
				...
				if (middlewaresRegex('passtlsclientcert.info.issuer', linePrefix)) {
					return completeOptions(
						Object.keys(
							dockerKeys.http.middlewares.passtlsclientcert.info.issuer
						)
					);
				}
				...
				if (middlewaresRegex('retry', linePrefix)) {
					return completeOptions(
						Object.keys(dockerKeys.http.middlewares.retry)
					);
				}
